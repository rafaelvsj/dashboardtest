...
from python_helper.api.src.service import LogHelper, ReflectionHelper, ObjectHelper, StringHelper
...
def Enum(instanceLog=False, associateReturnsTo=ENUM_VALUE_AS_STRING_KEY):
    def Wrapper(OuterEnum, *args, **kwargs):
        ...
        if instanceLog :
            LogHelper.log(Enum,f'''wrapping {OuterEnum.__name__}''')
        class InnerEnum(OuterEnum, EnumClass):
            ...
            def __init__(self,*args,**kwargs):
                ...
                if instanceLog :
                    LogHelper.prettyPython(OuterEnum, 'originalClassAttributeValueList', originalClassAttributeValueList)
                ...
                if instanceLog :
                    LogHelper.prettyPython(OuterEnum, 'attributeDataList', attributeDataList)
                for attribute, enumValue in attributeDataList :
                    ...
                    if instanceLog :
                        LogHelper.prettyPython(OuterEnum, 'attributeAttributeDataList', attributeAttributeDataList)
            ...
            def map(enumItemOrEnumItemValue):
                if instanceLog :
                    LogHelper.log(OuterEnum,f'''enumItemOrEnumItemValue: {enumItemOrEnumItemValue}''')
                if ObjectHelper.isNone(enumItemOrEnumItemValue):
                    ...
                    mappedEnum = OuterEnum.__enumMap__.get(
                        str(enumItemOrEnumItemValue) if not ReflectionHelper.hasAttributeOrMethod(enumItemOrEnumItemValue, 'enumName') else enumItemOrEnumItemValue.enumName
                    )
                    return mappedEnum if ObjectHelper.isNotNone(mappedEnum) else __raiseEnumValueNotImplemented__(
...
def isEnum(possibleEnum):
    ...
    try :
        ...
        LogHelper.error(isEnum, f'Not possible to evaluate. Returning {itIs} by default', exception)
    return itIs
...
def isEnumItem(possibleEnumItem):
    ...
    try :
        ...
        LogHelper.error(isEnumItem, f'Not possible to evaluate. Returning {itIs} by default', exception)
    return itIs
